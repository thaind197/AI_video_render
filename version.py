...
def check_remote_version(custom_url: str = None) -> Dict[str, Any]:
    """
    Kiểm tra phiên bản ứng dụng và cấu hình từ xa qua RemoteConfigManager (Firebase Realtime Database REST API).
    """
    try:
        from remote_config import get_remote_config_manager
        mgr = get_remote_config_manager()
        status_info = mgr.check_app_status(custom_url=custom_url)
        ...
        return {
            ...
            "update_message": status_info.get("update_message", status_info.get("block_reason", "")),
