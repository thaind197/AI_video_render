...
async function fetchSettings() {
    try {
        ...
        if (res.ok) {
            ...
            const s = data.data;
            if (s) {
                document.getElementById("settings-api-key").value = s.gemini_api_key || "";
                document.getElementById("settings-veo-threads").value = s.max_concurrent_veo_jobs;
                document.getElementById("settings-ffmpeg-threads").value = s.max_concurrent_processing;
                document.getElementById("settings-post-threads").value = s.max_concurrent_post_jobs;
            }
        }
    } catch (err) {
        console.warn("Không kết nối được API Cài Đặt:", err);
    }
}

// Fetch Social login status
async function fetchSocialStatus() {
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            const s = data.data;
            updateSocialStatusTag("fb-session-tag", s.facebook);
            updateSocialStatusTag("tiktok-session-tag", s.tiktok);
            updateSocialStatusTag("x-session-tag", s.x);
        }
    } catch (err) {
        console.warn("Không lấy được trạng thái liên kết Social:", err);
    }
}

function updateSocialStatusTag(id, active) {
    ...
    if (active) {
        ...
        el.className = "ant-tag ant-tag-error";
...
function initForms() {
    ...
    if (settingsForm) {
        settingsForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            const gemini_api_key = document.getElementById("settings-api-key").value;
            const max_concurrent_veo_jobs = parseInt(document.getElementById("settings-veo-threads").value) || 5;
            const max_concurrent_processing = parseInt(document.getElementById("settings-ffmpeg-threads").value) || 4;
            const max_concurrent_post_jobs = parseInt(document.getElementById("settings-post-threads").value) || 2;
...
function initModal() {
    ...
    // Bind immediate posting buttons
    ...
    const triggerPost = async (platform) => {
        if (!currentPreviewJobId) return;
        try {
            const res = await fetch(`${API_BASE}/api/social/post`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ job_id: currentPreviewJobId, platform })
            });
            const data = await res.json();
            if (res.ok) {
                alert(`🚀 ${data.message}`);
    ...
    if (fbBtn) fbBtn.onclick = () => triggerPost("facebook");
    if (tiktokBtn) tiktokBtn.onclick = () => triggerPost("tiktok");
    if (xBtn) xBtn.onclick = () => triggerPost("x");
}
...
function openPreviewModal(jobId) {
    ...
    const videoPlayer = document.getElementById("preview-video-player");
    const videoMock = document.getElementById("preview-video-mock");

    if (job.status === "READY_TO_POST" || job.status === "PUBLISHED" || job.status === "PUBLISHING") {
        videoPlayer.src = `/videos/final_${job.id}.mp4`;
        ...
        document.getElementById("preview-mock-sub").textContent = job.voiceover_text || job.veo_prompt || "Đang xử lý kịch bản...";
    }
